...
export class LrcLibProvider implements LyricProvider {
  ...
  async find(query: LyricQuery): Promise<LyricResult | null> {
    ...
    if (query.duration !== undefined) {
      url.searchParams.set("duration", String(Math.round(query.duration)));
    }
    ...
    try {
      ...
      if (response.status === 404) {
        logger.debug({ provider: this.name, url: url.toString() }, "LRCLIB returned 404 Not Found");
        return null;
      }
      ...
      logger.error({ err: error, provider: this.name }, "LRCLIB fetch error");
      throw error;
    }
  }
}
